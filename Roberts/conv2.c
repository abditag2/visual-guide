    ...
    for (jc = 2; jc - 2 < arg1->size[1]; jc++) {
      ...
      for (ic = 1; ic - 1 < arg1->size[0]; ic++) {
        ...
        for (ja = 0; ja <= i2; ja++) {
          ...
          i3 = ia2 - ic;
          for (ia = 0; ia <= i3; ia++) {
            s += arg1->data[((ic + ia) + arg1->size[0] * (b_ja - 1)) - 1] * (1.0
              - (double)jb);
          }
        }

        c->data[(ic + c->size[0] * (jc - 2)) - 1] = s;
      }
    }
  }
}
    ...
    for (jc = 1; jc - 1 < arg1->size[1]; jc++) {
      ...
      for (ic = 2; ic - 2 < arg1->size[0]; ic++) {
        ...
        for (ja = 0; ja <= i0; ja++) {
          ...
          for (ia = 0; ia <= i1; ia++) {
            b_ia = ia1 + ia;
            s += arg1->data[(b_ia + arg1->size[0] * b_ja) - 1] * (1.0 - (double)
              (ic - b_ia));
          }
        }

        c->data[(ic + c->size[0] * (jc - 1)) - 2] = s;
      }
    }
  }
}
